...
export const VocabLibraryModal: React.FC<VocabLibraryModalProps> = ({
  ...
  // Extract unique categories sorted by CEFR Level (A1 -> A2 -> B1 -> B2)
  const levelOrder: Record<string, number> = { 'A1': 1, 'A2': 2, 'B1': 3, 'B2': 4 };
  const rawCategories: string[] = Array.from(new Set(vocabulary.map((v) => v.category)));
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg sm:text-xl font-black text-white font-['Almarai',sans-serif]">
                  مكتبة المفردات الشاملة (Vocabulary Bank) 📚
                </h2>
              ...
              <p className="text-xs text-amber-100 font-bold mt-0.5">
                تصفح كل كلمات المستويات (A1, A2, B1, B2) واستمع لنطقها المعتمد للتدرب عليها!
              </p>
            </div>
          </div>
          ...
          {filteredVocab.length === 0 ? (
            ...
            filteredVocab.map((vocab) => {
              ...
              return (
                ...
                >
                  <div>
                    {/* Top Row: Category & CEFR Badge & Audio trigger */}
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {vocab.cefrLevel && (
                          <span className="text-[10px] font-black text-white bg-[#214ECF] border border-black px-1.5 py-0.5 rounded-lg shadow-[1px_1px_0px_0px_#000]">
                            {vocab.cefrLevel}
                          </span>
                        )}
                      </div>
