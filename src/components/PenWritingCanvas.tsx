...
interface StrokeAnalysisBreakdown {
  ...
  overallScore: number;     // Weighted total (must be >= 90%)
...
export const PenWritingCanvas: React.FC<PenWritingCanvasProps> = ({
  ...
  const handleVerifyHandwriting = () => {
    ...
    guideCtx.font = '900 110px Outfit, sans-serif';
    ...
    if (numDots > 0 && numReal < Math.ceil(wordLen * 0.5)) {
      ...
      diagnosticMsg = 'نسبة تطابق طريقة رسم وبنية الكلمة لم تتجاوز الحد الأدنى المطلوب (90%).';
    }
    ...
    // STRICT PASS CONDITION: Match Score MUST be >= 90%
    if (overallScore >= 90) {
      setFeedback({
        isSuccess: true,
      ...
      setFeedback({
        isSuccess: false,
        msg: `نسبة تطابق طريقة الكتابة هي ${overallScore}% (المطلوب 90% فأكثر للنجاح). ${diagnosticMsg} 🎯`,
        score: overallScore,
        breakdown
      });
    }
  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm dir-rtl">
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2 mb-1.5">
            ...
            <span className="bg-amber-50 text-amber-900 border border-amber-200 text-xs font-black px-3 py-1 rounded-full flex items-center gap-1">
              <span>مطابقة ضربات الحروف (Stroke Matching) ≥ 90%</span>
            </span>
      ...
      <div className="relative w-full h-[280px] bg-slate-50 rounded-2xl border border-dashed border-slate-300 overflow-hidden flex items-center justify-center select-none">
        ...
        <div className="absolute top-3 left-4 z-10 bg-white/90 text-slate-700 text-[11px] font-semibold px-2.5 py-1 rounded-lg border border-slate-200 flex items-center gap-1.5 font-['Outfit']" dir="ltr">
          <span>LTR ➔</span>
          <span>Left to Right (Stroke Match Active)</span>
        </div>

        {/* Light Tracing Guide Background Overlay */}
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none" dir="ltr">
          <span 
            className="text-8xl sm:text-9xl font-black text-slate-200/80 font-[Outfit] tracking-widest text-center"
        ...
        <div className="absolute bottom-3 right-4 text-[11px] text-slate-500 font-bold pointer-events-none bg-white/90 px-3 py-1 rounded-xl border border-slate-200/80 shadow-2xs flex items-center gap-1.5">
          {isEraser ? (
            <>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <span className="text-[11px] font-bold text-slate-600 block mt-0.5">
                  {feedback.isSuccess
                    ? 'تم اجتياز معيار مطابقة طريقة رسم الحروف بنجاح (تجاوزت 90%).'
                    : 'لم يتم تحقيق درجة النجاح. يجب ألا تقل نسبة مطابقة طريقة الكتابة عن 90%.'}
                </span>
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-900 text-white border-2 border-black shadow-[3px_3px_0px_0px_#000]">
              <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
                ...
                <span className={`text-xs font-black font-['Outfit'] px-2.5 py-0.5 rounded-full border ${
                  feedback.breakdown.overallScore >= 90
                    ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500'
                    : 'bg-rose-500/20 text-rose-300 border-rose-500'
                }`}>
                  الإجمالي: {feedback.breakdown.overallScore}% {feedback.breakdown.overallScore >= 90 ? '✅ (ناجح)' : '❌ (غير كافٍ)'}
                </span>
              </div>
              ...
              <div className="w-full h-3 bg-slate-800 rounded-full overflow-hidden border border-slate-700 mb-4 p-0.5">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${
                    feedback.breakdown.overallScore >= 90 ? 'bg-emerald-500' : 'bg-rose-500'
                  }`}
