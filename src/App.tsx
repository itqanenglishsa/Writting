...
export default function App() {
  ...
  const rawCategories: string[] = Array.from(new Set(VOCABULARY_LIST.map((v) => v.category)));
  rawCategories.sort((a, b) => {
    const getLvl = (cat: string) => {
      const match = cat.match(/\b(A1|A2|B1|B2)\b/);
  ...
  const categoriesList = ['الكل', ...rawCategories];
  ...
  // User Gamification Stats State (Starts from Day 1 & 0 XP)
  ...
  const [currentLevelNumber, setCurrentLevelNumber] = useState<number>(() => {
    return getUserLevel(0).currentLevel.level;
  });
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm flex flex-col justify-between">
            ...
            <div className="space-y-3.5 my-1">
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between text-xs">
                <span className="font-semibold text-slate-700">المتبقي لإتقان جميع المستويات:</span>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8 flex flex-col justify-between flex-1 relative overflow-hidden">
            ...
            <div className="flex flex-wrap items-center justify-between pb-4 border-b border-slate-100 mb-6 gap-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer"
            ...
            <div className="mt-8 border-t border-slate-100 pt-4 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-500 font-medium">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-slate-700">منهجية إتقان المباشرة لتعلم مفردات A1, A2, B1, B2</span>
              </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-800 uppercase tracking-wider font-['Almarai',sans-serif]">
                  شريط المفردات السريع (الكلمة {VOCABULARY_LIST.findIndex(v => v.id === activeVocabWord.id) + 1} من {VOCABULARY_LIST.length} • متبقي {Math.max(0, VOCABULARY_LIST.length - (VOCABULARY_LIST.findIndex(v => v.id === activeVocabWord.id) + 1))} كلمة)
                </span>
              </div>
              <button
