...
import { loadUserProgress, saveUserProgress, calculateStreak, getTodayDateString } from './utils/storage';
...
export default function App() {
  // Load saved progress once from Local Storage
  const [initialProgress] = useState(() => loadUserProgress());

  // Strategy Navigation State
  const [activeStrategy, setActiveStrategy] = useState<WritingStrategy>(initialProgress.activeStrategy);

  // Anti-Cheating & Focus View Mode State
  const [isStrictMode, setIsStrictMode] = useState<boolean>(initialProgress.isStrictMode);
  const [isFocusMode, setIsFocusMode] = useState<boolean>(initialProgress.isFocusMode);

  // Stories & Vocab Data State
  const [storiesList, setStoriesList] = useState<PracticeStory[]>(() => {
    if (initialProgress.customStories && initialProgress.customStories.length > 0) {
      const customIds = new Set(initialProgress.customStories.map((s) => s.id));
      const filteredDefaults = PRACTICE_STORIES.filter((s) => !customIds.has(s.id));
      return [...initialProgress.customStories, ...filteredDefaults];
    }
    return PRACTICE_STORIES;
  });

  const [activeStory, setActiveStory] = useState<PracticeStory>(() => {
    const customStories = initialProgress.customStories || [];
    const all = [...customStories, ...PRACTICE_STORIES];
    return all.find((s) => s.id === initialProgress.activeStoryId) || PRACTICE_STORIES[0];
  });

  const [activeVocabWord, setActiveVocabWord] = useState<VocabularyWord>(() => {
    return VOCABULARY_LIST.find((v) => v.id === initialProgress.activeVocabWordId) || VOCABULARY_LIST[0];
  });

  const [dockCategory, setDockCategory] = useState<string>(initialProgress.dockCategory);
  ...
  const [stats, setStats] = useState<UserStats>(initialProgress.stats);
  const [lastVisitDate, setLastVisitDate] = useState<string>(() => {
    return initialProgress.lastVisitDate || getTodayDateString();
  });
  const [rewardedTaskIds, setRewardedTaskIds] = useState<string[]>(() => {
    return initialProgress.rewardedTaskIds || [];
  });

  // Keep streakDays synchronized with real device calendar date
  useEffect(() => {
    const syncStreak = () => {
      const { streakDays, lastVisitDate: updatedVisitDate } = calculateStreak(lastVisitDate, stats.streakDays);
      setStats((prev) => {
        if (prev.streakDays !== streakDays) {
          return { ...prev, streakDays };
        }
        return prev;
      });
      if (updatedVisitDate !== lastVisitDate) {
        setLastVisitDate(updatedVisitDate);
      }
    };

    syncStreak();

    const handleFocus = () => syncStreak();
    window.addEventListener('focus', handleFocus);
    const interval = setInterval(syncStreak, 3600000); // Check hourly for midnight transition

    return () => {
      window.removeEventListener('focus', handleFocus);
      clearInterval(interval);
    };
  }, [lastVisitDate, stats.streakDays]);

  // Toasts Notifications State & Tracked Badges and Levels
  const [toasts, setToasts] = useState<ToastMessage[]>([]);
  const [unlockedBadgeIds, setUnlockedBadgeIds] = useState<string[]>(() => {
    if (initialProgress.unlockedBadgeIds && initialProgress.unlockedBadgeIds.length > 0) {
      return initialProgress.unlockedBadgeIds;
    }
    return BADGES_LIST.filter(b => b.checkUnlocked(initialProgress.stats)).map(b => b.id);
  });

  const [currentLevelNumber, setCurrentLevelNumber] = useState<number>(() => {
    return getUserLevel(initialProgress.stats.xp).currentLevel.level;
  });

  // Automatically sync progress to Local Storage on state changes
  useEffect(() => {
    const customStories = storiesList.filter(
      (s) => !PRACTICE_STORIES.some((ps) => ps.id === s.id)
    );
    ...
    saveUserProgress({
      ...
      activeVocabWordId: activeVocabWord.id,
      isStrictMode,
      isFocusMode,
      dockCategory,
      customStories,
  ...
  const handleRewardXp = (amount: number, taskId?: string) => {
    const effectiveTaskId = taskId || `${activeStrategy}_${activeStory.id}_${activeVocabWord.id}`;

    if (rewardedTaskIds.includes(effectiveTaskId)) {
      // Task already rewarded! Do not award duplicate XP.
      return;
    }

    setRewardedTaskIds((prev) => [...prev, effectiveTaskId]);
